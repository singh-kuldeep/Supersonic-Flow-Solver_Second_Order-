...
void BC(vector<vector<vector<vector<float> > > > & variablesvector , vector<vector<vector<vector<float> > > > & y_face_area, int Nx, int Ny, int Nz)
{
	...
	float totaltemperature = 518.76 ;
	float totalpressure = 792766.8;
	...
	for (int j =2; j < Ny-2; ++j)
	{
		...
		{
			float inletpressure = (gamma-1)*(variablesvector[2][j][k][4] - 0.5*(pow(variablesvector[2][j][k][1],2) +
			...
			variablesvector[0][j][k][1] = inletdensity*inletvelocity ;
			variablesvector[0][j][k][2] = 0 ;
			variablesvector[0][j][k][3] = 0 ;
			...
			variablesvector[1][j][k][1] = inletdensity*inletvelocity ;
			variablesvector[1][j][k][2] = 0 ;
			variablesvector[1][j][k][3] = 0 ;
	...
	for (int i = 2; i < Nx-2; ++i)
	{
		for (int k = 2; k < Nz-2; ++k)
		{
			float y_face_area_unit_vector[3] = {0.0,1.0,0.0} ;
			//using leftcell(j=2) filling j=1
			y_face_area_unit_vector[0] = y_face_area[i][2][k][0] / sqrt( pow(y_face_area[i][2][k][0] ,2) + pow(y_face_area[i][2][k][1] ,2) ) ;  
			y_face_area_unit_vector[1] = y_face_area[i][2][k][1] / sqrt( pow(y_face_area[i][2][k][0] ,2) + pow(y_face_area[i][2][k][1] ,2) ) ;
			y_face_area_unit_vector[2] = 0 ;
			...
			y_face_area_unit_vector[0] = y_face_area[i][3][k][0] / sqrt( pow(y_face_area[i][3][k][0] ,2) + pow(y_face_area[i][3][k][1] ,2) ) ;  
			y_face_area_unit_vector[1] = y_face_area[i][3][k][1] / sqrt( pow(y_face_area[i][3][k][0] ,2) + pow(y_face_area[i][3][k][1] ,2) ) ;
			y_face_area_unit_vector[2] = 0 ;
			...
			y_face_area_unit_vector[0] = - y_face_area[i][Ny-3][k][0] / sqrt( pow(y_face_area[i][Ny-3][k][0] ,2) + pow(y_face_area[i][Ny-3][k][1] ,2) ) ;  
			y_face_area_unit_vector[1] = - y_face_area[i][Ny-3][k][1] / sqrt( pow(y_face_area[i][Ny-3][k][0] ,2) + pow(y_face_area[i][Ny-3][k][1] ,2) ) ;
			y_face_area_unit_vector[2] = - 0 ;
			...
			// here every term has been multiplied by -1 because cell area vector is -A
			y_face_area_unit_vector[0] = - y_face_area[i][Ny-4][k][0] / sqrt( pow(y_face_area[i][Ny-4][k][0] ,2) + pow(y_face_area[i][Ny-4][k][1] ,2) ) ;  
			y_face_area_unit_vector[1] = - y_face_area[i][Ny-4][k][1] / sqrt( pow(y_face_area[i][Ny-4][k][0] ,2) + pow(y_face_area[i][Ny-4][k][1] ,2) ) ;
			y_face_area_unit_vector[2] = - 0 ;
	...
	for (int i = 2; i < Nx-2; ++i)
	{
		for (int j = 2; j < Ny-2; ++j)
		{
			...
			float z_face_area_unit_vector[3] ={0.0,0.0,1.0};
			// z_face_area_unit_vector[1] = 0;
			// z_face_area_unit_vector[2] = 1;
