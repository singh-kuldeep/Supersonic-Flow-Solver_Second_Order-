...
{
	public:
	float densityinterface ;
	float xvelocityinterface ;
	float yvelocityinterface ;
	float zvelocityinterface ;
	float enthalpyinterface ;

	float vectorjumpinterface[5] ;
	float eigenvalue[5] ;
	...
	float alphavectorinterface[5] ; // eigenvectormatrixinvers[5][5]*vectorjumpinterface

	float muvectorinterface[5] ; // this is some paramere = delta t * eigenvalue
	float Zvectorinterface[5] ; // this is dame as muvectorinterface

	float pshivectorinterface[5] ;
	float gvectorinterface[5];


	interface(vector<float>& vectorleft,vector<float>& vectorright,vector<float>& areavectorinterface,
				float volumeleft, float volumeright, float deltat) 
	{
		float volumeinterface = 0.5*(volumeleft + volumeright) ;

		float pressureleft = (gamma -1)*( vectorleft[4] - 0.5*(pow(vectorleft[1],2)+pow(vectorleft[2],2)+
			pow(vectorleft[3],2))/vectorleft[0] ) ;
		float pressureright = (gamma -1)*( vectorright[4] - 0.5*(pow(vectorright[1],2)+pow(vectorright[2],2)+
			pow(vectorright[3],2))/vectorright[0] ) ;
		
		float enthalpyleft = (vectorleft[4] + pressureleft)/vectorleft[0];
		float enthalpyright = (vectorright[4] + pressureright)/vectorright[0];
		...
		// engenvalues 
		float hx = areavectorinterface[0] /volumeinterface ;
		float hy = areavectorinterface[1] /volumeinterface ;
		float hz = areavectorinterface[2] /volumeinterface ;
		float hn = sqrt(pow(areavectorinterface[0],2) + pow(areavectorinterface[1],2) + 
			pow(areavectorinterface[2],2)) / volumeinterface ;
		float Ucont = xvelocityinterface*hx + yvelocityinterface*hy + zvelocityinterface*hz ;
		float soundvelocityinterface = sqrt((gamma -1)*(enthalpyinterface - 0.5*(pow(xvelocityinterface,2)+
		...
		float hdesy = hy/hn ; 
		float hdesz = hz/hn ; 
		float Phi = xvelocityinterface*hdesx + yvelocityinterface*hdesy + zvelocityinterface*hdesz ;
		float q = sqrt(pow(xvelocityinterface,2)+pow(yvelocityinterface,2)+pow(zvelocityinterface,2)) ;
...
// 	float deltaf = 0.2 ; // this is given constant value (it is between 0.1 to 0.5)
// 		for (int i = 0; i < 5; ++i)
// 		{
